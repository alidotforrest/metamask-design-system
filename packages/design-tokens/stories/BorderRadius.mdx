...
Both Tailwind presets replace Tailwind's default `borderRadius` scale with these
tokens, so `rounded-lg` and the other stock names produce nothing. `rounded-full`
survives because it is one of these tokens.
...
The default names no longer generate anything. Each maps to exactly one token at
the same value, so moving across is a rename with no visual change.
